...
plot(StackClimatex)
# Remove NA
Abundance <- na.omit(Abundance)#Remove NA completely
StackClimatex <- na.omit(StackClimatex)#Remove NA completely

#initialise
sim_dataSF <- initialise(
  n1_map =Abundance ,#represents NLmap
  K_map =StackClimatex, # represents Kmap Lu can be 2006 or 2019 or climate scenarios
  r = log(2),
  rate = 1 / 1e3
)
